...
function definirConteudoPreco(status, id, preço) {
    if (status == "ESGOTADO") {
        return `
        <span id="valor-${id}" class="w-full pt-0.5 pb-3 text-[#A42628] text-sm border-2 border-[#A42628] ">
            ESGOTADO
        </span>
        `;
    }
    ...
    for (let i = 0; i < listaConjuntos.length; i++) {
        let conteudo = `
            <div class="flex flex-col items-center text-center">
                <img src="${listaConjuntos[i].img}" alt="${listaConjuntos[i].nome}" class="max-h-3/5 ">
